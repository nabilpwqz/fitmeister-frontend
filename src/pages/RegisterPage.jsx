  ...
  return (
    <div className="mx-auto max-w-md px-4 py-20 sm:px-6">
      <div className="rounded-[2rem] border border-zinc-800 bg-zinc-950 p-10 shadow-2xl shadow-black/20">
        ...
        <p className="mt-3 text-sm text-zinc-400">Create your FitMeister account to book classes and join the community.</p>
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-5">
          <label className="block text-sm font-semibold text-zinc-300">
            Full Name
            <input value={name} onChange={(e) => setName(e.target.value)} type="text" required className="mt-2 w-full rounded-3xl border border-zinc-800 bg-zinc-900 px-4 py-3 text-white outline-none focus:border-brand-500" />
          </label>
          <label className="block text-sm font-semibold text-zinc-300">
            Email
          ...
          <button type="submit" className="w-full rounded-full bg-brand-500 px-5 py-3 text-sm font-semibold text-zinc-950 transition hover:bg-brand-400">Register</button>
        </form>
        <p className="mt-6 text-center text-sm text-zinc-500">Already have an account? <Link className="text-brand-300 hover:text-white" to="/login">Login</Link></p>
      </div>
    </div>
  );
};
